...
SpreadSheet.fn = SpreadSheet.prototype = {
    ...
    getLetterNum(str) {
        ...
        arr.forEach((value, index) => {
            num += (str.charCodeAt(str.length - index - 1) - 64) * Math.pow(26, index)
        })
        return num - 1
    },
    ...
    getPoint(point) {
        ...
        if (typeof point === 'string') {
            let regNumber = /^\d*$/
            let regLetter = /^[A-Z]*$/// 正则判断数字与大写字母
            if (regNumber.test(point)) { // 判断传入数字，表示传入行
                startRow = endRow = Number(point) - 1
            } else if (regLetter.test(point)) { // 判断传入字母，表示传入列
                let num = 0
            ...
            point.forEach((value, index) => {
                letter[index] = _this.getLetterNum(value.match(/^[A-Z]+/gi).toString())
    ...
    setFontColor(color, point) {
        ...
        this.setFont(p, { content: { color: color }}, 'color')
    },

    // 设置单元格背景
    setFillColor(color, point) {
        let p = this.getPoint(point)
        this.setFont(p, { content: { background: color }}, 'background')
    },

    // 批量设置单元格背景色
    batchFillBg(color, points) {
        points.forEach((point, index) => {
            let p = this.getPoint([point.startCol + point.startRow, point.endCol + point.endRow])
            this.setFont(p, { content: { background: color }}, 'background')
        })
    },
    ...
    setFontFamily(name, point) {
        let p = this.getPoint(point)
        this.setFont(p, { content: { family: name }}, 'family')
    },

    // 设置字体大小
    setFontSize(size, point) {
        let p = this.getPoint(point)
        this.setFont(p, { content: { size: size }}, 'size')
    },
    ...
    setFontWeight(weight, point) {
        ...
        this.setFont(p, { content: { weight: bool }}, 'weight')
    },
    ...
    setFontStyle(style, point) {
        ...
        this.setFont(p, { content: { italic: bool }}, 'italic')
    },
    ...
    setAlign(align, point) {
        ...
        if (align === 'left' || align === 'right' || align === 'center') {
            this.setFont(p, { content: { alignRow: align }}, 'alignRow')
        }
        else {
            this.setFont(p, { content: { alignCol: align }}, 'alignCol')
        }
    },
    ...
    destroyDataSoure() {
        ...
        this.vm.$store.commit(M_types.DELETE_SELECT, { select: destroyDataSource })
    },

    // 使用相对于浏览器页面坐标获取在表格中行列位置
    getPointByPosi(clientX, clientY) {
        let colIndex = String.fromCharCode(this.vm.$store.getters.getColIndexByPosi(clientX) + 65)
        let rowIndex = this.vm.$store.getters.getRowIndexByPosi(clientY) + 1
        return {
            point: {
                col: colIndex,
                row: rowIndex
