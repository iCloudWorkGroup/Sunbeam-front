    ...
    mounted() {
        ...
        if (!frozenRule || frozenRule.type === 'mainRule') {
            this.$store.commit(mutationTypes.UPDATE_USERVIEW, {
                ...
                bottom: this.offsetTop + this.$el.clientHeight + config
                    .prestrainHeight
            });
        }
    },
    ...
    methods: {
        ...
        scrollToBottom(limitTop, limitBottom, resolve) {
            ...
            function getNextRow(resolve) {
                ...
                if (currentMaxBottom < limitBottom) {
                    ...
                    limitBottom = limitBottom < maxBottom ? limitBottom :
                        maxBottom;
                    ...
                    self.verticalRequest(currentMaxBottom + 1, limitBottom,
                        ...
                        function(alias) {
                            ...
                            for (let i = occupyBottomIndex + 1, len =
                                    rowRecord.length; i < len; i++) {
                                temp.push(rowRecord[i]);
                                rowOccupy.push(rowRecord[i]);
                            }
                            temp.push(alias);

                            for (let i = 0, len1 = colOccupy.length - 1; i <
                                len1; i++) {
                                for (let j = 0, len2 = temp.length - 1; j <
                                    len2; j++) {
                                    let sign = colOccupy[i] + '_' +
                                        colOccupy[i + 1] + '_' +
                                        temp[j] + '_' + temp[j + 1];
                ...
                if (occupyBottom < limitBottom) {
                    ...
                    for (let i = 0, len1 = temp.length - 1; i < len1; i++) {
                        for (let j = 0, len2 = colOccupy.length - 1; j < len2; j++) {
                            let sign = colOccupy[j] + '_' + colOccupy[j + 1] +
                                '_' +
                                temp[i] + '_' + temp[i + 1];
                            if (!regionRecord.get(sign)) {
                                regionRecord.set(sign, true);
                    ...
                    if (flag) {
                        self.verticalRequest(occupyBottom + 1, limitBottom,
                            resolve);
                    } else {
                        resolve();
                    }
        ...
        scrollToRight(limitLeft, limitRight, resolve) {
            let colList = this.$store.getters.colList,
                ...
                occupyEndCol = this.$store.getters.getColByAlias(
                    occupyEndColAlias),
             ...
             * 需要自动增加列
             */
            if (limitRight > maxRight && (!frozenRule || frozenRule.type ===
                    'mainRule')) {
                addColNum = Math.ceil((limitRight - maxRight + bufferWidth) /
                    config.colWidth);
                limitRight = maxRight;
            }
                ...
                if (currentMaxRight < limitRight) {
                    ...
                    self.transverseRequest(currentMaxRight + 1, limitRight,
                        ...
                        function(alias) {
                            ...
                            for (let i = occupyRightIndex, len = colRecord.length; i <
                                len; i++) {
                            ...
                            for (let i = 0, len1 = rowOccupy.length - 1; i <
                                len1; i++) {
                                for (let j = 0, len2 = temp.length - 1; j <
                                    len2; j++) {
                                    let sign = temp[j] + '_' + temp[j + 1] +
                                        '_' +
                                        rowOccupy[i] + '_' + rowOccupy[i +
                                            1];
                                    if (!regionRecord.get(sign)) {
                                        regionRecord.set(sign, true);
                                    }
            ...
            function getRight(resolve) {
                if (occupyRight < limitRight) {
                    ...
                    if (flag) {
                        self.transverseRequest(occupyRight + 1, limitRight,
                            resolve);
                    } else {
                        resolve();
                    }
                    ...
                    if (data.spreadSheet && data.spreadSheet[0] &&
                        ...
                        if (fn) {
                            let cols = sheetData.glX,
                                endColAlias = cols[cols.length - 1]
                                .aliasX;
                        ...
                        this.$store.dispatch(actionTypes.CELLS_RESTORECELL,
                            cells);
                    }
                    resolve();
                }
